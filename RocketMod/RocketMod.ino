 ...
 *
 * Connect a variable resistor to analog input pin (potPin) to control
 * rocket position
 * A piezo (or speaker through a series resistor) can be connected to
 * (speakerPin) for sound 
 *
 * Arduino forum discussion thread here:
 * http://www.arduino.cc/cgi-bin/yabb2/YaBB.pl?num=1265067548
 *
 * 2013-06.15 bperrybap   - updates and conversion to openGLCD
 */


#include <openGLCD.h>
#include "bitmaps.h"         // rocket game bitmaps
#include <EEPROM.h>
...
#define potPin        A5  // analog input pin connected to a variable resistor   
#define speakerPin    2   // digital pin that can optionally connected to a piezo or speaker for sound

//#define brightness = 64; // set backlight brightness
 ...
 */
#define random(min,max) (uint8_t)random(min,max)

#define EEPROM_Cell 52
...
uint8_t rockAmount = 6;
unsigned int record = 0;
bool record_beat = false;
...
void setup(){
  GLCD.Init();
#ifdef brightness // want to set brightness?
  GLCD.SetBacklight(brightness); // set backlight brightness
#endif
#ifdef speakerPin
...
// Read record
   EEPROM_readAnything(EEPROM_Cell, record);
...
#else
/*
 * No pot input so move the rocket up and down
 */
static uint8_t yval;
...
void updatePos(int entity){
    if (millis() - speedMillis[entity-1] > tracker[entity][3]) {
      ...
      if(tracker[entity][1] < 1){
        score++;
        level++;
        level = constrain(level, 0, 75);
        if(level > 75){
          fastSpeed --;
          fastSpeed = constrain(fastSpeed, 10, 25);
        }
        
        tracker[entity][0] = random(1, 8);   // bitmap
        tracker[entity][1] = 119;            // start point
        tracker[entity][2] = random(1, 56);  // height
...
******************************
*/
boolean collision(int8_t entity){
  for(int8_t playerScanCOL = 0; playerScanCOL<=9; playerScanCOL++){
    if(tracker[0][1]+playerScanCOL == tracker[entity][1]+1){
      for(int8_t playerScanROW = -1; playerScanROW < 6; playerScanROW  ++){
      for(int8_t entityScanROW = -1; entityScanROW < 6; entityScanROW  ++){
...
*/
void gameOver(){  
  if(record_beat)EEPROM_writeAnything(EEPROM_Cell, record);
